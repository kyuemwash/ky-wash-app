...
import React, { useState, useEffect } from 'react';
import { Bell, Plus, X, LogOut, User, Clock, Droplet, Wind, Settings, AlertCircle, CheckCircle, TrendingUp, Wrench, Moon, Sun, Lock, Flag, Activity } from 'lucide-react';
...
// Real-time database with cross-device sync
...
const KYWash = () => {
  ...
  const [activeTab, setActiveTab] = useState<'washer' | 'dryer' | 'activity'>('washer');
  ...
  const [activityLog, setActivityLog] = useState<Array<{id: number, user: string, action: string, machine: string, timestamp: string}>>([]);
  ...
  const sendPushNotification = (userData: User, type: string, machineId: number) => {
    ...
    // Store notification persistently for all devices and users
    ...
    const fullNotification = {
      ...
      isGlobal: true, // Mark for global visibility
    };
    allNotifications.push(fullNotification);
    localStorage.setItem('kywash_all_notifications', JSON.stringify(allNotifications));

    // Broadcast notification to all tabs and devices
    try {
      const bc = new BroadcastChannel('kywash_notifications');
      bc.postMessage({
        type: 'notification',
      ...
      console.log('BroadcastChannel not supported for notifications');
    }
    ...
    if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
      try {
        new Notification('KY Wash - Cycle Complete! 🎉', {
          ...
          data: { machineId, studentId: userData.studentId }
        });
  ...
  // Listen for notifications from other devices/tabs
  useEffect(() => {
    try {
      const notificationChannel = new BroadcastChannel('kywash_notifications');
      notificationChannel.onmessage = (event) => {
        if (event.data?.type === 'notification') {
          // Display notification on this device too
          const notif = event.data.notification;
          if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
            try {
              new Notification('KY Wash Alert 📢', {
                body: `${notif.user.studentId}'s ${notif.type} #${notif.machineId} is ready!`,
                icon: '/laundry-icon.png',
      ...
      console.log('Notification broadcast channel not available');
    }
  }, []);

  // Sync machine state across devices
  useEffect(() => {
    try {
      const syncChannel = new BroadcastChannel('kywash_sync');
      syncChannel.onmessage = (event) => {
        if (event.data?.type === 'update') {
          // Force re-render by updating localStorage
          localStorage.setItem('kywash_db', JSON.stringify(event.data.payload));
        }
      };
      return () => syncChannel.close();
    } catch (e) {
      console.log('Sync broadcast channel not available');
    }
  }, []);

  // Listen for activity updates from other devices
  useEffect(() => {
    try {
      const activityChannel = new BroadcastChannel('kywash_activity');
      activityChannel.onmessage = (event) => {
        if (event.data?.type === 'activity') {
          const newActivity = event.data.activity;
          // Add activity to log if not already there (prevent duplicates)
          setActivityLog(prev => {
            if (prev.some(a => a.id === newActivity.id)) {
              return prev; // Already have this activity
            }
            return [newActivity, ...prev].slice(0, 50);
          });
        }
      };
      return () => activityChannel.close();
    } catch (e) {
      console.log('Activity broadcast channel not available');
    }
  }, []);
  ...
  const logActivity = (action: string, machineType: string, machineId: number) => {
    if (!user) return;
    ...
    // Persist to localStorage for cross-device visibility
    const allActivities = JSON.parse(localStorage.getItem('kywash_activity_log') || '[]');
    allActivities.unshift({
      ...activity,
      fullTimestamp: new Date().toISOString(),
    });
    localStorage.setItem('kywash_activity_log', JSON.stringify(allActivities.slice(0, 200))); // Keep last 200
    ...
    try {
      ...
      console.log('Activity broadcast failed');
    }
  };
  ...
  const handleReportFault = () => {
    ...
    // Log this activity globally
    logActivity(`Reported issue on ${reportMachine.type}`, reportMachine.type.charAt(0).toUpperCase() + reportMachine.type.slice(1), reportMachine.id);
  ...
  const handleStartMachine = () => {
    ...
    // Log this activity globally
    const timeText = cycle.time === 30 ? '30 min' : `${cycle.time} min`;
    logActivity(`Started ${machine.type}`, machine.type.charAt(0).toUpperCase() + machine.type.slice(1), machine.id);
  ...
  const handleEndCycle = (machineId: number, machineType: string) => {
    ...
    // Log this activity globally
    logActivity(`Ended ${machineType} cycle`, machineType.charAt(0).toUpperCase() + machineType.slice(1), machineId);
  };
  ...
  const handleClothesCollected = (machineId: number) => {
    ...
    // Log this activity globally
    logActivity(`Collected clothes from ${machine.type}`, machine.type.charAt(0).toUpperCase() + machine.type.slice(1), machineId);
  };
  ...
  const handleJoinWaitlist = (type: string) => {
    ...
    // Log this activity globally
    logActivity(`Joined ${type} waitlist`, type.charAt(0).toUpperCase() + type.slice(1), 0);
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4 border-b overflow-x-auto">
              <button
                ...
                className={`py-3 px-6 font-semibold text-lg transition border-b-2 whitespace-nowrap ${
                  activeTab === 'washer'
            ...
            {activeTab === 'activity' && (
              <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-lg p-6`}>
                <h2 className={`text-2xl font-bold mb-6 ${darkMode ? 'text-white' : 'text-black'}`}>
                  <Activity className="inline w-6 h-6 mr-2 text-green-600" />
                ...
                {activityLog.length === 0 ? (
                  <div className={`text-center py-12 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    ...
                    <p>No activities yet. Start using the laundry machines!</p>
                  </div>
                ) : (
                  <div className="space-y-3 max-h-[600px] overflow-y-auto">
                    {activityLog.map((activity) => (
                      <div 
                        key={activity.id} 
                        className={`p-4 rounded-lg border ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'} hover:shadow-md transition`}
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                              {activity.action} - <span className="font-medium">{activity.machine}</span>
                            </div>
                          </div>
                          <div className={`text-xs font-medium ml-4 flex-shrink-0 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                            {activity.timestamp}
                          </div>
                        </div>
                      </div>
